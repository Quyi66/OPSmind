...
6. 步骤级执行日志写入 `vap2_patch_operation_log`，审计日志写入 `jao_audit_log`（与 Linux 共用）
...
Windows 补丁任务的执行日志与 Linux 补丁任务共用 `vap2_patch_operation_log` 表，可通过 Linux 侧已有的审计日志查询接口查看：
...
```
GET /api/vap/v2/patch/task/{taskId}/audit/detail
```
...
```json
{
  ...
  "steps": [
    ...
    { "step": "VALIDATE",  "label": "校验",     "status": "PENDING", "remark": "...", "runId": null, "logs": [...] }
  ],
  "logs": [ ... ]
}
```

> `vap2_patch_operation_log` 中每条记录的 `task_type` 字段区分 Linux（`install`/`rollback`）和 Windows（`INSTALL`/`ROLLBACK`/`SCAN`）任务类型。
