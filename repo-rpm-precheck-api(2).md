...
本模块提供补丁包可用性预检功能。在下发补丁安装之前，先将补丁涉及的包名+版本传给目标主机，由目标主机在本地 yum/dnf 仓库中定点查询这些包是否可安装，返回逐包检查结果。

**核心思路：** 拿着补丁要求的具体包列表，到目标机器上查仓库能不能装——能装才装，不能装就跳过提示。兼容联网/离线/本地仓库等各种场景，支持 RHEL、CentOS、麒麟、openEuler、SUSE 等主流发行版。

**预检作用范围：**

| 任务类型 | 是否预检 | 说明 |
|---------|---------|------|
| 补丁安装 (install) | 是 | 检查补丁关联的 RPM 包在目标主机仓库中是否可用 |
| 漏洞修复 (vuln_fix) | 是 | 同上，漏洞修复本质是安装补丁 |
| 软件包更新 (pkg_update) | 是 | 检查指定的包名在目标主机仓库中是否可用 |
| 补丁回滚 (rollback) | **否** | 回滚使用 yum downgrade 还原旧版本 |

> 如果任务无需检查的包（如无关联 PatchAffectedPkg），预检会直接通过，不影响流程。
...
将待检查的包列表下发到目标主机，在目标主机的 yum/dnf 仓库中逐包查询是否可安装。
...
```json
{
  ...
  "packages": [
    {"name": "openssl", "version": "1.1.1w-1.ky10"},
    {"name": "curl", "version": "7.88.1-1.ky10"},
    {"name": "glibc"}
  ]
}
```
...
| packages | object[] | 是 | 待检查的包列表 |
| packages[].name | string | 是 | RPM 包名 |
| packages[].version | string | 否 | 要求的版本（version-release 格式），不填则只检查包是否存在 |
...
**请求体：** 标准 `JaoCallbackData` 格式，`batches[].result.scanResultFile` 指向预检结果 JSON 文件路径。
...
{
  "hostId": "acm-host-001",
  "scanTime": "2026-04-15T10:30:00Z",
  "osInfo": {"distro": "Kylin", "version": "V10", "arch": "x86_64"},
  "pkgManager": "dnf",
  "totalChecked": 3,
  "availableCount": 2,
  "unavailableCount": 1,
  "checkResults": [
    {"name": "openssl", "requiredVersion": "1.1.1w-1.ky10", "available": true, "status": "AVAILABLE", "availableVersions": ["1.1.1w-1.ky10"]},
    {"name": "curl", "requiredVersion": "", "available": true, "status": "AVAILABLE", "availableVersions": ["7.88.1-1.ky10"]},
    {"name": "libxml2", "requiredVersion": "2.12.0", "available": false, "status": "VERSION_MISMATCH", "availableVersions": ["2.9.7-16.ky10"]}
  ]
}
```

**checkResults[].status 枚举值：**

| 值 | 含义 | 说明 |
|---|------|------|
| AVAILABLE | 仓库中可用 | 包存在且版本满足要求（或未指定版本要求） |
| VERSION_MISMATCH | 版本不匹配 | 包在仓库中存在，但没有要求的版本 |
| NOT_FOUND | 包不存在 | 在所有已启用仓库中均未找到此包 |

---

## 四、与创建任务接口的集成

以下已有接口在创建任务后执行预检步骤时，会自动从补丁关联的 `PatchAffectedPkg` 提取包名+版本列表，下发到目标主机仓库中查询可用性：

| 接口 | 预检行为 |
|------|---------|
| `POST /api/vap/v2/patch/task/create` | 从 patchIds 提取关联包，到目标主机仓库查可用性 |
| `POST /api/vap/v2/patch/task/create-vuln-fix` | 同上 |
| `POST /api/vap/v2/patch/task/create-pkg-update` | 将指定包名发到目标主机仓库查可用性 |
| `POST /api/vap/v2/patch/task/create-rollback` | **不预检**（回滚不依赖仓库） |

**预检失败时的错误信息（记录在任务的 errorMessage 中）：**

```
以下包在仓库中不可用: openssl, libxml2
```

**前端建议处理方式：**

1. 在创建任务前，先调用 `POST /api/vap/v2/patch/repo-check/quick-check` 做前置检查
2. 如果 `passed=false`，展示不满足的包列表，提示用户检查主机仓库配置
3. 如果 `passed=true` 或任务无关联包，正常创建任务
4. 也可以直接创建任务，预检步骤会在回调中自动判定通过/失败
