    ...
    loadingPromise = (async () => {
        ...
        try {
            ...
            const importPromises = modules.map(async (module) => {
                try {
                    const response = await fetch(`/oplus-modules/src/webapp/i18n/zh-cn/${module}.json`)
                    if (response.ok) {
                        const data = await response.json()
                        Object.assign(translations, data)
                    }
                } catch (e) {
                    console.warn(`Failed to load i18n module: ${module}`, e)
                }
            })

            await Promise.all(importPromises)
        } catch (error) {
            console.warn('Failed to load i18n translations:', error)
        }
