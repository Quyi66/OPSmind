<template>
  <div class="ops-page-layout">
    ...
    <div class="ops-table-wrapper">
      <el-table :data="sudoTemplates" v-loading="loadingTemplates" max-height="calc(100vh - 250px)">
        ...
        <el-table-column label="操作" width="110" fixed="right">
          <template #default="{ row }">
            <el-button text type="primary" size="small" @click="handleViewTemplate(row)">
              修改
            </el-button>
            <el-button text type="danger" size="small" @click="confirmDeleteTemplate(row)">
              删除
            </el-button>
          </template>
        </el-table-column>
        ...
        <div class="command-section">
          ...
          <el-table :data="templateCommands" size="small" v-loading="loadingCommands">
            ...
            <el-table-column label="操作" width="80">
              <template #default="{ row }">
                <el-button text type="danger" size="small" @click="confirmDeleteCommand(row)">
                  删除
                </el-button>
              </template>
            </el-table-column>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="addingCommand" @click="handleAddCommand">
          添加
        </el-button>
      </template>
    </el-dialog>
...
async function loadSudoTemplates() {
  ...
  try {
    const response = await sudoApi.getSudoTemplates({
...
async function handleCreateTemplate() {
  ...
  try {
    await sudoApi.createSudoTemplate({
      id: null,
...
async function loadTemplateCommands() {
  ...
  try {
    const response = await sudoApi.getSudoCommandsByTemplate(currentTemplate.value.id, {
...
async function handleDeleteTemplate(template) {
  try {
    await sudoApi.deleteSudoTemplate(template.id)
...
async function handleAddCommand() {
  ...
  try {
    await sudoApi.createSudoCommand({
...
async function confirmDeleteCommand(command) {
  try {
    await ElMessageBox.confirm(`确定要删除命令「${command.command}」吗？`, '删除确认', {
      type: 'warning',
      confirmButtonText: '删除',
      cancelButtonText: '取消'
    })
...
async function handleDeleteCommand(command) {
  try {
    await sudoApi.deleteSudoCommand(command.id)
