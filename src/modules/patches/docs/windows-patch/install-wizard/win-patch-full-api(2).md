...
Windows 补丁任务的执行日志与 Linux 补丁任务共用 `vap2_patch_operation_log` 表。Windows 侧直接提供与 Linux 同口径的审计查询接口：
...
返回 `List<PatchOperationLog>`，按 `seqNo` 升序。
...
**响应结构（与 Linux 审计详情结构保持一致）**

```json
{
  "task": { "id": "...", "taskType": "INSTALL", "taskStatus": "COMPLETED", "currentStep": "COMPLETED", "...": "..." },
  "steps": [
    { "step": "PRE_CHECK", "label": "预检查",   "status": "SKIPPED", "remark": "...", "runId": null,          "logs": [...] },
    { "step": "INSTALL",   "label": "补丁安装", "status": "SUCCESS", "remark": "...", "runId": "jao-run-xxx", "logs": [...] },
    { "step": "RESTART",   "label": "重启",     "status": "SKIPPED", "remark": "...", "runId": null,          "logs": [...] },
    { "step": "VALIDATE",  "label": "脚本校验", "status": "PENDING", "remark": "...", "runId": null,          "logs": [...] }
  ],
  "logs": [ ... ]
}
```

> `vap2_patch_operation_log.task_type` 均写入小写（`install` / `rollback`），便于 Linux / Windows 共用 `buildAuditDetail` 等聚合逻辑。Windows 任务可通过 `task` 实体或 `taskSnapshot` 字段识别。
