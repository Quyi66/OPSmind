...
以补丁为维度聚合展示比对结果，每个补丁显示所需包的统计、不满足的包明细、以及该补丁影响的机器数。

```
GET /patch-compare/{diffRunId}/patch-view?keyword=kernel&status=NOT_SATISFIED&page=0&size=20
```

| 参数 | 必填 | 说明 |
|------|------|------|
| diffRunId | **是** | 比对运行 ID |
| keyword | 否 | 搜索关键字，匹配 patchId 或补丁标题 |
| diffType | 否 | 只返回存在该类型差异的补丁（MISSING / OUTDATED / RELEASE_MISMATCH / AVAILABLE） |
| status | 否 | SATISFIED — 仅满足的补丁；NOT_SATISFIED — 仅不满足的补丁 |
| page | 否 | 页码（默认 0） |
| size | 否 | 每页条数（默认 20） |
  ...
  "content": [
    {
      ...
      "patchTitle": "Important: kernel security update",
      "severity": "Important",
      "satisfied": false,
      "totalPkgs": 5,
      "availableCount": 3,
      ...
      "failedPackages": [
        {
          "pkgName": "kernel",
          "diffType": "OUTDATED",
          "requiredNevra": "kernel.x86_64 5.14.0-362.24.1.el9_3",
          "baselineNevra": "0:5.14.0-362.8.1.el9_3-362.8.1.el9_3"
        },
        {
          "pkgName": "kernel-tools",
          "diffType": "MISSING",
          "requiredNevra": "kernel-tools.x86_64 5.14.0-362.24.1.el9_3",
          "baselineNevra": null
        }
      ]
    }
...
|------|------|
| patchId | 补丁 ID |
| patchTitle | 补丁标题（来自 vap2_patch） |
| severity | 严重等级 |
| satisfied | 该补丁所有所需包是否都 AVAILABLE |
| totalPkgs | 该补丁关联的总包数 |
| availableCount | 满足版本要求的包数 |
| missingCount | 仓库中缺失的包数 |
| outdatedCount | 仓库中版本不够的包数 |
| releaseMismatchCount | version 相同但 release 不匹配的包数 |
| affectedHostCount | 需要修复该补丁的主机数（来自扫描结果） |
| failedPackages | 不满足的包明细列表（仅包含非 AVAILABLE 的包） |

> **前端建议**：默认按 `affectedHostCount` 降序排列（接口已内置），可结合 `status=NOT_SATISFIED` 快速筛出需要关注的补丁。
...
④ 补丁比对页
   ...
   └→ 展示汇总：通过数/缺失数/版本不够数
   └→ 展示补丁维度列表（GET /patch-view）— 推荐，按补丁聚合显示，含影响机器数
   └→ 支持搜索（keyword）、筛选（status=NOT_SATISFIED）
   └→ 展开单个补丁可查看不满足的包明细（failedPackages 字段已内联返回）
```
