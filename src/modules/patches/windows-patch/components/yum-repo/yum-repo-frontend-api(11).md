...
| 采集触发 | 必须前端手动点「采集」按钮 | 仍由前端显式调用 `POST /collect`；`POST /configs` 退化为纯 INSERT，不再隐式触发采集 |
| 比对触发 | 必须前端手动点「比对」按钮 | 仍由前端显式调用 `POST /patch-compare/scanned`；采集 SUCCESS 后后端不再隐式触发比对 |
...
> 5. **「新增 YUM 源 → 采集 → 比对」三步链路由前端显式编排**：`POST /configs` 拿到 `dcDataId` 后，前端按需调 `POST /collect { dcDataId }`，再轮询 `GET /repos/{sourceId}/status` 直到 `SUCCESS`/`FAILED`，最后按需调 `POST /patch-compare/scanned { sourceId }` 拿 `diffRunId` 跳转结果页。后端不再隐式串联这三步，便于在 UI 中分步展示进度与就近报错。
...
> **本接口仅做 INSERT，不再隐式触发采集**。前端拿到 `dcDataId` 后，按需在同一交互流里再调 `POST /collect { dcDataId }` 启动采集；返回成功后开始轮询 `GET /repos/{sourceId}/status` 等 `SUCCESS`/`FAILED`。
> 推荐流程：`POST /configs` → `POST /collect` → 轮询 `GET /repos/{sourceId}/status` → `POST /patch-compare/scanned`（如需立即比对）。每一步独立可重试、独立可观测，不再像旧版那样隐式串联导致单一步骤失败时前端无感知。
...
> **`baseurl` 变更行为**：当请求体中的 `baseurl` 与现有记录不同时，后端会按 (tenant + 旧 baseurl) 找到对应的 RepoSource 并**级联清理**它的快照（snapshot）、包（package）、差异（diff/diff_run）以及 RepoSource 自身，避免在 `vap2_repo_source` 留下孤儿源污染 `/patch-compare/overview`。前端在「编辑后想看新数据」时，需要重新调一次 `POST /collect { dcDataId }` 触发采集。
...
3. 失败：snapshot 标记 `FAILED`，错误信息截断后写入 `error_message`。

> 采集 SUCCESS 后是否触发补丁比对，由前端通过 `POST /patch-compare/scanned` 显式调用决定，后端不再隐式串联。
...
⓪ YUM源配置页
   ...
   └→ 点击"YUM源配置录入" → 新增（POST /configs）→ 拿到 dcDataId 后建议立刻继续 ① 引导用户走"采集 → 比对"
...
> 「新增 → 采集 → 比对」三步建议封装成同一个交互向导：
>   step1: POST /configs                      → { dcDataId }
>   step2: POST /collect { dcDataId }         → { sourceId, snapshotId }
>   step3: 轮询 GET /repos/{sourceId}/status  → SUCCESS / FAILED
>   step4: SUCCESS 后 POST /patch-compare/scanned { sourceId } → { diffRunId }
>   step5: 跳转 GET /patch-compare/{diffRunId}/patch-view 结果页
> 任意一步失败，前端就近显示 errorMessage 并允许重试当前步骤；后端各步均为幂等，可放心重试。
