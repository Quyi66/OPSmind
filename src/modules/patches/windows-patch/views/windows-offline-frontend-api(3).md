...
| 2.0c | 扫描 | POST | `/vap/api/vap/win-patch/tasks/scan` | PRIVUSER |
...
> 「扫描」触发见 2.0c；目标机本地采集（不依赖 WSUS），完成后由 JAO 回调 `/callback/scan`（3.1）落库，后端再用知识库比对算出缺失补丁。
...
### 2.0c 触发扫描 `POST /vap/api/vap/win-patch/tasks/scan`
对指定主机下发 `func=scan` 作业，目标机**本地采集**已装 KB + 系统 build（不依赖 WSUS / 联网），完成后由 JAO 回调 `/win/callback/scan`（3.1）落库，后端再用知识库 `vap2_win_patch_library` 比对算出缺失补丁。

- Body：`["hostId1","hostId2"]` 或 `{ "hostIds": ["hostId1","hostId2"] }`（两种均兼容）。
- 角色：`ROLE_PRIVUSER`。
- 响应：`{ "_status": "ok", "runId": "..." }`；`hostIds` 为空返回 400 `{"_status":"error","message":"hostIds 不能为空"}`，派发失败返回 500。
- 异步：接口立即返回，真实进度看 `operation_log`（轮询）或回调落库后刷新主机列表。

cURL：

```bash
curl -X POST 'https://host/oplus-portal/vap/api/vap/win-patch/tasks/scan' \
  -H 'Authorization: Bearer <token>' -H 'Content-Type: application/json' \
  -d '["uuid-host-1","uuid-host-2"]'
```
