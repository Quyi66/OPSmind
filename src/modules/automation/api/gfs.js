...
export const updateFileInfo = (repoType, repo, path, fileInfo) => {
  const cacheBuster = Date.now()
  return useApi().put(
    `/gfs/api/gfs/v2/${repoType.toUpperCase()}/f/${getRepo(repo)}/file/${path}?cacheBuster=${cacheBuster}`,
    fileInfo
  )
}
...
export const downloadFile = (repoType, repo, path, saveFilename) => {
  const url = withApiBaseUrl(
    `/gfs/api/gfs/v2/${repoType}/r/${getRepo(repo)}/download/${encodeURIComponent(path)}`
  )
...
export const rollbackFileRevision = (repo, commitName, path) => {
  return useApi().post(
    `/gfs/api/gfs/v2/git/r/${getRepo(repo)}/rev/${encodeURIComponent(path)}/${commitName}`
  )
}
...
export const getFileApprovalHistory = (repoType, repo, filePath) => {
  return useApi().get(
    `/gfs/api/gfs/v2/${repoType}/f/${getRepo(repo)}/history/${encodeURIComponent(filePath)}`
  )
}
...
export const getFileApproveHistory = (repo, filePath) => {
  return useApi().get(
    `/gfs/api/gfs/v2/git/f/${getRepo(repo)}/history/${encodeURIComponent(filePath)}`
  )
}
...
export const changeFileStatus = (repoType, repo, files, status, comment) => {
  ...
  const cacheBuster = Date.now()
  return useApi().put(
    `/gfs/api/gfs/v2/${repoType}/f/${getRepo(repo)}/attr?cacheBuster=${cacheBuster}`,
    {
      onlineStatus: status,
      comment,
      srcPaths: filePaths
...
export const initCurrentGitRepo = repo => {
  const cacheBuster = Date.now()
  return useApi().post(`/gfs/api/gfs/v2/git/f/${getRepo(repo)}/init?cacheBuster=${cacheBuster}`, {
