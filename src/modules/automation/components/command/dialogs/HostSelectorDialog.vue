...
async function loadHostData() {
  try {
    // 通过 udp 数据集获取主机列表
    const res = await useApi().post('/udp/api/udp/dataset/ACM_GET_CI_BY_SELECTOR/page', {
      pageNum: 1,
      pageSize: 200,
      params: {
        groups: '@@',
    ...
    if (res?.data?.list || res?.list) {
      const list = res.data?.list || res.list || []
      allHosts.value = list.map(h => ({
        id: h.id,
        key: h.IP || h.ip || h.id,
        value: h.IP || h.ip || h.id,
        ip: h.IP || h.ip,
        hostname: h.hostname || h.name,
        os: h.os || h.operatingSystem,
        osVersion: h.osVersion || h.version,
        status: h.status === 1 || h.status === 'online' ? 'online' : 'offline',
        connectRate: h.connectRate,
        owner: h.owner,
...
async function loadGroupData() {
  try {
    // 源系统 API: /api/acm/query/group/view/{assetType}
    const res = await useApi().get('/acm/api/acm/query/group/view/linux').catch((err) => {
      console.error('分组API错误:', err)
      return null
    })
    console.log('分组数据原始响应:', res)
    if (res) {
      // axios 返回的数据在 res.data 中
      const groupPaths = res.data || []
      console.log('分组路径:', groupPaths)
      if (Array.isArray(groupPaths) && groupPaths.length > 0) {
        ...
        console.log('分组树数据:', groupTreeData.value)
      }
    }
  } catch (error) {
...
async function loadTagData() {
  try {
    // 源系统 API: /api/acm/query/tag/view/{assetType}
    const res = await useApi().get('/acm/api/acm/query/tag/view/linux').catch((err) => {
      console.error('标签API错误:', err)
      return null
    })
    console.log('标签数据原始响应:', res)
    if (res) {
      // axios 返回的数据在 res.data 中
      const tags = res.data || []
...
async function loadRecentlyData() {
  try {
    const res = await useApi().post('/jao/api/jao/jobs/recently', {
...
function removeHost(index) {
  ...
  if (hostTableRef.value) {
    const row = filteredHosts.value.find(h => h.value === host.value)
    if (row) {
...
function handleConfirm() {
  emit('update:modelValue', [...selectedHosts.value])
