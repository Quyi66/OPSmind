...
「YUM 源配置录入」弹窗新增三个字段，分别对应后端 `osFamily` / `osMajor` / `osSpVersion`。
...
- **格式校验**（仅在用户填写时校验，留空跳过；麒麟必填判断在前一道）：
  ...
  - 不允许两端空格、长度 1~32；
  - **正则**：`^(SP\d+( \w+)?|Update\d+|HPC|Host|Compat)$`，
    覆盖：`SP1`、`SP2`、`SP3`、`SP3 2403`、`SP3 2309b`、`Update6`、`HPC`、`Host`、`Compat`。
  - 大小写敏感地落库（后端不做大小写归一），建议前端按上述形式提交。

- 校验提示（按情况选一）：
  - 未填且 osFamily 是 kylinos：`麒麟仓库必须填写 OS 精确版本（如 SP1、SP3、SP3 2403、HPC、Host）`
  - 填写了但格式错：`OS 精确版本格式不正确，请按 SP1 / SP3 2403 / Update6 / HPC / Host / Compat 形式填写`
...
```json
{ "success": false, "message": "麒麟仓库必须填写「OS精确版本」（如 SP1、SP3、SP3 2403、HPC、Host），否则采集到的包无法与 vap2_patch 精确比对，会出现跨 SP 误报" }
```
...
- `osSpVersion`：**手动输入**，需与 `os_sp_version` **完全相等**，麒麟（`vendor`/`osFamily` 为 `kylinos`）必填，
  正则 `^(SP\d+( \w+)?|Update\d+|HPC|Host|Compat)$`；
  `SP3` 与 `SP3 2403` 视为不同。
...
| ------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| `name`        | 输入     | 非空                                                                                                 |
| `baseurls`    | 多行输入 | 至少 1 条非空；提交前 `trim` 并去掉空行；建议同地址去重；建议 `^https?://` 开头                       |
...
| `osSpVersion` | 输入     | `osFamily=kylinos` 时必填；填写后须匹配 `^(SP\d+( \w+)?|Update\d+|HPC|Host|Compat)$`                 |
| `arch`        | 下拉     | 建议必选；可选值见上节                                                                               |
| `file`        | 输入     | 可选                                                                                                 |
...
3. 服务包：仓库 `osSpVersion` 与补丁包 `os_sp_version` **严格相等**。
...
- `POST /patch-compare/scanned`：先 `vap2_curr_machine_patch JOIN vap2_curr_machine_scan`，
  按 `(os_distro, os_major_version, os_sp_version, os_arch)` 四元组等值匹配仓库的
...
2. 提示客户去 YUM 源配置页面正确填写仓库的 `osFamily/osMajor/osSpVersion/arch`，
   配置一次后所有比对都按这个走。
