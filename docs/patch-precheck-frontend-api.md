...
- **不可达主机**：SSH/Ansible 通道不通的主机，等价于 BLOCKER，单独列在 `unreachable` 数组里（用主机在派发时的全集与实际存活主机差集识别）。
...
`preCheckResult` 为字符串化 JSON，解析后结构如下：
...
| 字段 | 说明 |
| --- | --- |
| `func` | 固定 `"check"`，用于区分回调类型 |
| `results[]` | 每台**可达**主机一条 |
| `results[].host_id` | 主机 CI id（与创建任务时的 hostIds 对应） |
| `results[].conn` | 连通性，可达主机恒为 `"ok"` |
| `results[].blockers` | 该主机阻断项数量 |
| `results[].warnings` | 该主机警告项数量 |
| `results[].checks[]` | 检查项明细 |
| `checks[].id` | 检查项标识，见第 4 节 |
| `checks[].status` | `ok` / `fail`(BLOCKER) / `warn`(WARNING) |
| `checks[].detail` | 该项的中文说明/失败原因 |
| `unreachable[]` | 不可达主机的 `host_id` 列表（等价 BLOCKER） |

> 判定是否通过：`unreachable` 为空 **且** 所有 `results[].blockers == 0`。
...
1. 顶部先渲染 `unreachable`（若非空）：红色「不可达主机（N 台）」，列出 host_id（可映射为主机名/IP）。
2. 按 `results[]` 逐台分组展示，主机标题旁标注 `阻断 N / 警告 M`。
3. 组内检查项按严重度排序：`fail` 置顶（红色），`warn` 其次（黄色），`ok` 折叠（绿色）。
4. 底部提供两个操作：
...
3. 轮询 `GET .../{id}`：
   ...
   - 失败：`status: "PRE_CHECK_FAILED"`，`error_message` 例：
     `不可达主机(1): ci-1002。主机 ci-1001 阻断项: version_ok(以下目标版本在仓库中不可解析: openssl-1.0.2k-23.el7_9)。`
