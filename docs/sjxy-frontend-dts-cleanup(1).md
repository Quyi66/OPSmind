...
- **现状**：后端请求 `/opsMind/i18n/sjxy-lang.js`，但前端容器内文件仍叫旧名 `oplus-lang.js`，返回 404（162 上已临时复制改名救急，但镜像重建后会丢失）。
- **改法**：前端构建产物中把 `oplus-lang.js` 改名为 `sjxy-lang.js`（含引用处），随下次发版一并上线。

## 6. 操作日志模块名传参

- **现状**：前端查询操作日志时仍传旧模块名（`acm`、`vap2` 等），后端 `workflow_audit_log.module` 存量数据因此保留旧值未迁移。
- **改法**：前端统一改传新模块名（cmdb、secops 等，与 API 前缀一致）；上线时通知后端同步 UPDATE `workflow_audit_log.module` 存量数据。
