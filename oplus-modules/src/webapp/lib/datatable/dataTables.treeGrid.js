...
(function (factory) {
        ...
        fnConstruct: function (oInit) {
            ...
            dataTable.on('click', 'td.treegrid-control', function (e) {
                ...
                if (data.children && data.children.length) {
                    ...
                    var td = $(dataTable.cell(rowSelector, '.tg-control').node());
                    ...
                    data.children.forEach(function (item, index) {
                        ...
                        $(node).attr('parent-index', parentTrId);
                        $(node).attr('tg-id', getTrId(index));
                    ...
                    if (dataTable.fixedColumns) {
                        // dataTable.draw();
                        dataTable.context[0].aiDisplay = dataTable.context[0].aiDisplayMaster.slice();
            ...
            dataTable.on('click', 'td.treegrid-control-open', function (e) {
                ...
                if (dataTable.fixedColumns) {
                    // console.log(dataTable.context[0].aiDisplay);
                    dataTable.context[0].aiDisplay = dataTable.context[0].aiDisplayMaster.slice();
    ...
    function getTrId(flag) {
        return 'tr-' + Date.now() + '-' + Math.ceil(Math.random() * 10000) + '-' + flag;
    }
    ...
    $(document).on('init.dt.treeGrid', function (e, settings) {
        ...
        //set id for all tr
        $('>tbody>tr', settings.nTable).each(function (i, tr) {
            tr.setAttribute('tg-id', getTrId(i));
            //
            //
            //     console.log('Add in the tbody elements, cloning form the master table');
            //     var i = that.s.dt.oFeatures.bServerSide === false ?
            //         that.s.dt.aiDisplay[that.s.dt._iDisplayStart + z] : z;
            //     //aoData元素书序和界面上的顺序不一致
            //     // var aTds = that.s.dt.aoData[i].anCells || $(this).children('td, th');
            //     var aTds = $(this).children('td, th') || that.s.dt.aoData[i].anCells;
            //
