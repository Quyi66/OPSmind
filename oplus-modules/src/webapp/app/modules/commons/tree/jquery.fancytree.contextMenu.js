 ...
 * LEO@20201008: Modified to add events
 */
(function ($, document) {
    "use strict";

    var initContextMenu = function (tree, selector, menu, actions, options) {
        tree.$container.on("mousedown.contextMenu", function (event) {
            ...
            if (node) {
                ...
                $.contextMenu({
                    ...
                    events: {
                        show: function (opt) {
                            opt.prevKeyboard = tree.options.keyboard;
                            tree.options.keyboard = false;
                            if (options.showEvent){
                                return options.showEvent(opt);
                            }
                        },
                        hide: function (opt) {
                            tree.options.keyboard = opt.prevKeyboard;
                            node.setFocus(true);
                        },
                    },
