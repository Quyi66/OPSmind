...
(function () {
    ...
    angular.module('oplus.udp').run(['echartsWidgetBuilder', 'widgetFactory', 'chartUpdater', 'widgetInteraction', 'widgetDataUtil', 'themeService', '$timeout', 'i18nService', 'conditionalFormat', 'dataEx',
        function (chartWidgetBuilder, widgetFactory, chartUpdater, widgetInteraction, widgetDataUtil, themeService, $timeout, i18nService, conditionalFormat, dataEx) {
            widgetFactory.defineWidget({
                type: 'dynamic-topology-chart',
                name: '拓扑图',
                group: 'data',
                resizable: 'h',
                configController: dynamicTopologyConfigController,
                controlRenderer: {
             ...
             * @param props
             */
            function dynamicTopologyConfigController(scope, props) {

                scope.nodeFields = [
                    {name: 'key', label: '主键'},
                    {name: 'category', label: '分类'},
                    {name: 'icon', label: '图标'},
                    {name: 'iconColor', label: '图标颜色'},
                    {name: 'iconSize', label: '图标大小'},
                    {name: 'text', label: '文字'},
                    {name: 'textColor', label: '文字颜色'},
                    {name: 'textSize', label: '文字大小'}
                ]
                scope.uwProps.lineRules = scope.uwProps.lineRules || [];
                scope.uwProps.lineRuleType = scope.uwProps.lineRuleType || 'fromData';

                scope.addLineRule = function () {
                    scope.uwProps.lineRules.push({
                        sourceRule: '',
                        targetRule: '',
                        lineWidth: 1,
                        lineColor: 'rgb(0,0,0)',
                ...
                scope.removeLineRule = function (index) {
                    scope.uwProps.lineRules.splice(index, 1);
                }
                widgetInteraction.upgradeWidgetProps(scope.uwProps);
            }
                ...
                if (widgetDataUtil.isDatasetConfigGood(props.dataset)) {
                    widgetDataUtil.queryData(elem, props).then(function (data) {
                        if (data.total > 0) {
                            ...
                            var echartsSeriesData = [];
                            var echartsSeriesLinks = [];
                            var echartsSeriesCategories = [];
                            scope.nodeFields = [
                                {name: 'key', label: '主键'},
                                {name: 'category', label: '分类'},
                                {name: 'icon', label: '图标'},
                            ...
                            var realField = {
                                key: _.get(props.fields, 'key.field', 'id'),
                                category: _.get(props.fields, 'category.field', 'category'),
                                icon: _.get(props.fields, 'icon.field', 'icon'),
                                iconColor: _.get(props.fields, 'iconColor.field', 'iconColor'),
                                iconSize: _.get(props.fields, 'iconSize.field', 'iconSize'),
                                text: _.get(props.fields, 'text.field', 'text'),
                                textColor: _.get(props.fields, 'textColor.field', 'textColor'),
                                textSize: _.get(props.fields, 'textSize.field', 'textSize')
                            }

                            _.forEach(props.lineRules, function (rule) {
                                if (rule.sourceRule && rule.targetRule) {
                                    var sourceFn = dataEx.strToFunc(rule.sourceRule);
                                    var targetFn = dataEx.strToFunc(rule.targetRule);
                                    if (_.isFunction(sourceFn) && _.isFunction(targetFn)) {
                                        var sourceNodes = _.filter(records, function (row) {
                                            return sourceFn.apply(this, [dataEx, row]);
                                        })
                                        var targetNodes = _.filter(records, function (row) {
                                            return targetFn.apply(this, [dataEx, row])
                                        })
                                        if (sourceNodes && sourceNodes.length > 0 && targetNodes && targetNodes.length > 0) {
                                            _.forEach(sourceNodes, function (sourceNode) {
                                                _.forEach(targetNodes, function (targetNode) {
                                                    echartsSeriesLinks.push({
                            ...
                            _.forEach(records, function (row, index) {
                                var categoryName = _.get(row, realField.category, '未分类');
                                var category = _.findIndex(echartsSeriesCategories, {name: categoryName});

                                if (category == -1) {
                                    echartsSeriesCategories.push({name: categoryName});
                                    category = echartsSeriesCategories.length - 1;
                                }
                                var node = {
                                    id: _.get(row, realField.key, ''),
                                    category: category,
                                    symbol: _.get(row, realField.icon, getDefaultSvgPah()),
                                    symbolSize: _.get(row, realField.iconSize, 40),
                                    itemStyle: {normal: {color: _.get(row, realField.iconColor, ''),}},
                                    name: _.get(row, realField.text, ''),
                                    label: {
                                        fontSize: _.get(row, realField.textSize, 12),
                                        color: _.get(row, realField.textSize, ''),
                                    }
                                }
                                echartsSeriesData.push(node);

                                if (row.link && _.isArray(row.link) && row.link.length > 0) {
                                    _.forEach(row.link, function (singeLink) {
                                        echartsSeriesLinks.push({
                                            source: _.get(row, realField.key, ''),
                                            target: singeLink.target,
                                            lineStyle: {
                                                normal: {
                                                    width: singeLink.lineWidth || 1,
                                                    color: singeLink.lineColor || 'source',
                                                    type: singeLink.lineColor || 'solid'
                                                }
                                            }
                                        });
                                    })
                            ...
                            option.series[0].data = echartsSeriesData;
                            option.series[0].links = echartsSeriesLinks;
                            option.series[0].categories = echartsSeriesCategories;
                            option.legend.data = echartsSeriesCategories;
            ...
            function getValueByField(fieldName, row) {
