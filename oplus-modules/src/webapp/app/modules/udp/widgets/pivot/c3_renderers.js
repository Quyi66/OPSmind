/**
 * @author leoliaolei@gmail.com, 2020/09/16, modified
 */
(function () {
    var callWithJQuery;
    ...
    callWithJQuery(function ($, c3) {
        ...
        makeC3Chart = function (chartOpts) {
            ...
            return function (pivotData, opts) {
                var agg, attrs, base, base1, base2, base3, base4, base5, base6, base7, c, categories, colKey, colKeys,
                    columns, dataColumns, defaults, formatter, fullAggName, groupByTitle, h, hAxisTitle, headers, i, j,
                    k, l, len, len1, len2, len3, len4, m, numCharsInHAxis, numSeries, params, ref, ref1, ref2, ref3,
                    renderArea, result, rotationAngle, row, rowHeader, rowKey, rowKeys, s, scatterData, series, title,
                    titleText, vAxisTitle, val, vals, x, xs, y;
                defaults = {
                    localeStrings: {
                        vs: "vs",
                        by: "by"
                ...
                // LEO@20200916: Use option to set title
                if (angular.isDefined(opts.titleText)) {
                ...
                //LEO@20200916: Use container to determine responsive chart size
                //https://codepen.io/michaellee/pen/eNodMx/
                if (opts.chartContainer) {
                    renderArea = opts.chartContainer;
                } else {
                    renderArea = $("<div>", {
                        style: "display:none;"
                    }).appendTo($("body"));
                }
                result = $("<div class='js-c3-bindto' style='width:100%;'></div>").appendTo(renderArea);
                params.bindto = result[0];
                //LEO: Remove fixed size
                delete params.size;
                params.size = {height: opts.chartContainer.height()};
                // console.log('c3.params:',opts.chartContainer.height(), params);
                c3.generate(params);
                result.detach();
                if (opts.chartContainer) {
                    // opts.chartContainer.find('svg').attr('width',Math.floor(result.width()));
                } else {
                    renderArea.remove();
                }
                return result;
                // return $("<div class='js-chart-title-wrapper'>").append(title, result);
            };
        };
