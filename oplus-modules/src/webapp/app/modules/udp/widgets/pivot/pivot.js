/**
 * Modified version of https://github.com/nicolaskruchten/pivottable/blob/master/dist/pivot.js
 *
 * Modifications are marked with LEOMOD
 *
 * @author leoliaolei@gmail.com, 2021/12/04, modified for better localization
 */
    ...
    callWithJQuery(function ($) {
        ...
        $.fn.pivotUI = function (input, inputOpts, overwrite, locale) {
            var a, aggregator, attr, attrLength, attrValues, c, colOrderArrow, defaults, e, existingOpts, fn1, i,
                ...
                rendererControl, rowOrderArrow, shownAttributes, shownInAggregators, shownInDragDrop, tr1, tr2, uiTable,
            ...
            try {
                ...
                for (x in ref) {
                    ...
                    // LEOMOD_BEGIN
                    // LEO@20211204: Make option value consistence for different locale.
                    // $("<option>").val(x).html(x).appendTo(renderer);
                    var label = !!opts.localeStrings.renderers ? (opts.localeStrings.renderers[x] || x) : x;
                    $("<option>").val(x).html(label).appendTo(renderer);
                    // LEOMOD_END
                }
                ...
                for (x in ref1) {
                    ...
                    var label = !!opts.localeStrings.aggregators ? (opts.localeStrings.aggregators[x] || x) : x;
                    aggregator.append($("<option>").val(x).html(label));
                    // LEOMOD_END
                }
                ordering = {
                    key_a_to_z: {
