...
export default function CustomElementFactory(bpmnFactory, moddle, translate) {
  ...
  this.create = function (elementType, attrs) {
      ...
      if (!attrs.businessObject) {
        ...
        if (attrs.typePrefix) { 
          assign(attrs.businessObject, {
            name: translate(attrs.typePrefix.replace(/.*:/, '')) + '_' + countType(self._bpmnFactory._model.ids._seed.hats, attrs.typePrefix.replace(/.*:/, '')),
            // id: camelCase(nextId(attrs.type.replace(/.*:/, '') + '_'))
            id: nextId(attrs.typePrefix.replace(/.*:/, '') + '_')
          })
        }
        else if (!attrs.businessObject.id) {
          assign(attrs.businessObject, {
            name: translate(attrs.type.replace(/.*:/, '')) + '_' + countType(self._bpmnFactory._model.ids._seed.hats, attrs.type.replace(/.*:/, '')),
            // id: camelCase(nextId(attrs.type.replace(/.*:/, '') + '_'))
            id: nextId(attrs.type.replace(/.*:/, '') + '_')
          })
        } else if (attrs.id) {
          assign(attrs.businessObject, {
            id: attrs.id
    ...
    var camundaKeys = Object.keys(attrs).filter(function (f) { return /^camunda:/.test(f) }); 

    if (camundaKeys.length > 0) {
      camundaKeys.forEach(function (k) {
        assign(attrs.businessObject, {
          [k.replace('camunda:', '')]: attrs[k]
        })
      })
    }

    // add type to businessObject if custom
    if (new RegExp(`^${eleConst.prefixEx()}`).test(type)) {
      // if (!attrs.businessObject) {
      ...
      //   } else {
      //     assign(attrs.businessObject, {
      //       id: nextId(attrs.type.replace(eleConst.prefixEx(), '') + '_')
      //     })
      //   }
      // }
...
function countType(obj, type) {
  ...
  for (var p in obj) {
    ...
    if (typeof (obj[p]) === 'function') {
      ...
      if (p.startsWith(type) || (obj[p].id && obj[p].id.startsWith(type))) {
        count += 1
      }
    }
  }
