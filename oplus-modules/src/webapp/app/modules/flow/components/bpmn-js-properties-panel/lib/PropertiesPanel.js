...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  // updateLabel(panelNode, '[data-label-id]', getBusinessObject(element).name || getBusinessObject(element).id || '');
  updateLabel(panelNode, '.label[data-label-id]', getBusinessObject(element).name || '');
  updateLabel(panelNode, '.subtitle[data-label-id]', getBusinessObject(element).id.substr(0, getBusinessObject(element).id.indexOf('_')) || '');
};
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  var panelNode = domify('<div class="bpp-properties"></div>'),
      headerNode = domify('<div class="bpp-properties-header">' +
        ...
        '<div class="subtitle" data-label-id></div>' +
      '</div>'),
