...
(function () {
    ...
    angular.module('oplus.jao').controller('flowStepCtrl', FlowStepCtrl);

    FlowStepCtrl.$inject = ['$scope', '$rootScope', '$state', '$timeout', '$uibModal', 'messageService', 'flowService', '$stateParams', '$translate'];
     ...
     * @constructor
     */
    function FlowStepCtrl($scope, $rootScope, $state, $timeout, $uibModal, messageService, flowService, $stateParams, $translate) {
        var that = this;
        ...
        function cancel() {
            $state.go('app.os.flow_list.instance_view', {id: that.step.flowId});
        }

        function onInit() {
            flowService.getStepAndHostStatus($stateParams.stepId).then(function (result) {
                ...
                result.hostList.forEach(function (hostStatus) {
                    ...
                    console.log($stateParams.selectedHosts);
        ...
        function runStep() {
            ...
            flowService.runStep(stepJob).then(function (result) {
                $state.go('app.os.flow_list.instance_view', {id: that.step.flowId});
            }).catch(function (err) {
