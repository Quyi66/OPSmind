...
(function () {
    ...
    angular.module('oplus.os').service('flowService', flowService);


    flowService.$inject = ['$q', '$http', '$interval', '$uibModal', '$state', 'restUtils', 'jaoUtil', '$translate', 'messageService'];
     ...
     */
    function flowService($q, $http, $interval, $uibModal, $state, restUtils, jaoUtil, $translate, messageService) {
        //Todo have't os module,use jao module
        var module = "jao";
        var that = this;
        ...
        function getHostStatusByStepIdAndHostKey(stepId, hostKey) {
            return restUtils.callApi(module, 'GET', "/api/jao/flows/{stepId}/hosts/{hostkey}/result", {stepId: stepId, hostkey: hostKey});
        }
        ...
        function saveFlow(flow) {
            // todo: 检验group_name及其group_vars
            // _.forEach(flow.steps, function (step) {
            //     step.config = JSON.parse(step.configJson);
            //     _.forEach(step.config.tasks, function (task) {
            //         _.forEach(task.groups, function (group) {
            //             var re = /^[0-9a-zA-Z_]{1,}$/;
            //             if (re.test(group.group)) {
            //                 messageService.toast("分组名只允许英文和数字下划线");
            //             }
            //             // todo: 校验group_vars 格式为 a=1 \n b=2
            //
            //         });
            //     });
            // });
